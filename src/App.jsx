...
function SectionLabel({ children }) {
  return (
    <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-3 py-1 font-mono text-xs uppercase tracking-[0.24em] text-vine-blue">
...
function GlowCard({ children, className = '' }) {
  return (
    <motion.div
      variants={fadeUp}
      className={`analog-panel group relative overflow-hidden rounded-lg border border-slate-200 p-6 shadow-analog backdrop-blur ${className}`}
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-vine-purple/70 via-vine-green/80 to-transparent opacity-80" />
      <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-vine-purple/10 blur-2xl transition duration-500 group-hover:bg-vine-green/20" />
...
function Hero() {
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden px-5 pt-24 sm:px-8 lg:px-12">
      <NetworkBackground />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(30,111,22,0.13),transparent_34%),radial-gradient(circle_at_78%_62%,rgba(90,46,166,0.11),transparent_28%)]" />
      ...
      >
        <div>
          <motion.div variants={fadeUp} className="mb-6 flex flex-wrap items-center gap-3">
            <span className="rounded-full border border-vine-green/30 bg-vine-green/10 px-3 py-1 font-mono text-xs uppercase tracking-[0.22em] text-vine-deepGreen">
              Tree of Connection / FAB26 MIT
            </span>
            <span className="rounded-full border border-vine-purple/30 bg-vine-purple/10 px-3 py-1 font-mono text-xs uppercase tracking-[0.22em] text-vine-purple">
              Living prototype
            </span>
          </motion.div>
          ...
          <motion.h1
            variants={fadeUp}
            className="title-glow font-display text-6xl font-semibold leading-none text-slate-950 sm:text-7xl md:text-8xl lg:text-9xl"
          >
            Fab Vine
          </motion.h1>

          <motion.p variants={fadeUp} className="mt-6 max-w-2xl text-2xl leading-tight text-vine-deepGreen md:text-3xl">
            A living network of fabrication, data, and human connection.
          </motion.p>

          <motion.p variants={fadeUp} className="mt-6 max-w-2xl text-base leading-8 text-slate-700 md:text-lg">
        ...
        <motion.div variants={fadeUp} className="relative mx-auto aspect-square w-full max-w-[540px]">
          ...
          <div className="absolute inset-12 rounded-full border border-vine-purple/25" />
          <div className="vine-orbit absolute inset-0 rounded-full border border-dashed border-slate-200" />
          <div className="absolute left-1/2 top-1/2 grid h-32 w-32 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-[1.7rem] border border-vine-green/40 bg-vine-moss/25 shadow-glow">
            <Leaf className="h-12 w-12 text-vine-deepGreen" />
          </div>
          ...
          <div className="absolute bottom-8 left-8 right-8 rounded-lg border border-vine-purple/15 bg-white/80 p-4 font-mono text-xs text-slate-700 backdrop-blur">
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          <motion.div variants={fadeUp}>
            <h2 className="font-display text-4xl font-semibold leading-tight text-slate-950 md:text-6xl">
...
function SpiralMethodology() {
  return (
    <section id="methodology" className="relative overflow-hidden px-5 py-24 sm:px-8 lg:px-12">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,rgba(30,111,22,0.1),transparent_32%),radial-gradient(circle_at_80%_60%,rgba(90,46,166,0.12),transparent_34%)]" />
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="relative mx-auto max-w-7xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr]">
          <motion.div variants={fadeUp}>
            ...
            <div className="mt-8 rounded-lg border border-vine-purple/20 bg-vine-purple/[0.06] p-5 font-mono text-sm text-vine-deepGreen">
...
function Fab26() {
  return (
    <section id="fab26" className="relative overflow-hidden px-5 py-24 sm:px-8 lg:px-12">
      <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(30,111,22,0.08),transparent_45%,rgba(22,75,160,0.08),rgba(90,46,166,0.07))]" />
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="relative mx-auto max-w-7xl">
        <SectionLabel>FAB26 Cambridge/MIT</SectionLabel>
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <GlowCard className="flex min-h-72 flex-col justify-between">
            <div className="flex h-24 w-24 items-center justify-center rounded-lg border border-vine-purple/30 bg-vine-purple/10 font-mono text-sm uppercase tracking-[0.2em] text-vine-purple">
              FAB26
            </div>
            <div>
              <h2 className="font-display text-4xl font-semibold text-slate-950">Cambridge / MIT 2026</h2>
          ...
          <motion.div variants={stagger} className="grid gap-4 md:grid-cols-3">
            {spiralSteps.map((step) => (
              <GlowCard key={step.title}>
                ...
                <h3 className="mt-8 font-display text-2xl font-semibold text-slate-950">{step.title}</h3>
                <p className="mt-4 text-sm leading-7 text-slate-600">{step.result}</p>
              </GlowCard>
            ))}
          </motion.div>
        </div>
        <motion.p variants={fadeUp} className="mt-10 max-w-4xl text-2xl leading-snug text-slate-800">
  ...
  return (
    <GlowCard className="lg:col-span-2">
      <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div>
          <div className="font-mono text-xs uppercase tracking-[0.2em] text-vine-purple">Interactive behavior console</div>
          <h3 className="mt-4 font-display text-3xl font-semibold text-slate-950">Test the swarm engine logic.</h3>
        ...
        <div className="relative min-h-80 overflow-hidden rounded-lg border border-vine-purple/15 bg-white/80 p-6">
          <div
          ...
          <div className="relative grid h-full place-items-center">
            <div className="relative h-56 w-56">
              ...
              >
                <span className="font-display text-4xl font-semibold text-slate-950">{swarm}</span>
                <span className="font-mono text-xs uppercase tracking-[0.16em] text-vine-green">swarm</span>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-3 font-mono text-xs text-slate-600">
              <span>capture</span>
              <span>normalize</span>
    ...
    <section id="technology" className="px-5 py-24 sm:px-8 lg:px-12">
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="mx-auto max-w-7xl">
        ...
        <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <h2 className="max-w-3xl font-display text-4xl font-semibold leading-tight text-slate-950 md:text-6xl">
            From leaf node inputs to color-coded swarm behavior.
          </h2>
          <div className="rounded-lg border border-slate-200 bg-white/80 px-4 py-3 font-mono text-xs text-slate-600">
        ...
        <motion.div variants={fadeUp}>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {audiences.map((audience) => (
              <span key={audience} className="rounded-full border border-vine-green/20 bg-vine-green/[0.08] px-3 py-1 text-sm text-vine-deepGreen">
                {audience}
              </span>
            ))}
          </div>
          ...
          <form className="grid gap-4 sm:grid-cols-2">
            {['Name', 'Country', 'Fab Lab / Organization', 'Skills', 'GitHub username', 'Email'].map((label) => (
              <label key={label} className="block">
                <span className="mb-2 block font-mono text-xs uppercase tracking-[0.18em] text-slate-500">{label}</span>
                <input
                  type={label === 'Email' ? 'email' : 'text'}
                  placeholder={label}
                  className="w-full rounded-lg border border-slate-200 bg-white/80 px-4 py-3 text-slate-950 outline-none transition placeholder:text-slate-950/25 focus:border-vine-green/70 focus:shadow-glow"
                />
              </label>
            ))}
            <button type="button" className="mt-2 inline-flex items-center justify-center gap-2 rounded-full bg-vine-purple px-6 py-3 font-semibold text-white transition hover:-translate-y-0.5 hover:bg-vine-blue sm:col-span-2">
              Register Interest
  ...
  return (
    <footer className="border-t border-slate-200 px-5 py-10 sm:px-8 lg:px-12">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="font-display text-2xl font-semibold text-slate-950">Fab Vine</div>
...
export default function App() {
  return (
    <main className="min-h-screen overflow-hidden bg-vine-paper text-slate-950">
      <Hero />
      <About />
      <SpiralMethodology />
      <Fab26 />
