...
void drawMatrixEyes(const unsigned long& currentMillis) {
  const float phase = (float)(currentMillis % MATRIX_GLOW_MS) / (float)MATRIX_GLOW_MS;
  const float breath = (sinf((phase * TWO_PI) - HALF_PI) + 1.0f) * 0.5f;
  const uint8_t glow = 145 + (uint8_t)(70.0f * breath);
  const uint16_t eyeColor = screen.Color(glow, glow / 3, glow / 4);
  const uint16_t pupilColor = screen.Color(18, 2, 3);

  screen.clear();

  if (currentExpression == EyesBlink) {
    screen.drawFastHLine(0, 4, 3, eyeColor);
    screen.drawFastHLine(5, 4, 3, eyeColor);
  } else if (currentExpression == EyesWink) {
    screen.drawFastHLine(0, 4, 3, eyeColor);
    drawMatrixEye(5, 2, 1, eyeColor, pupilColor);
  } else {
    int8_t pupilOffset = 1;

    if (currentExpression == EyesLookLeft) {
      pupilOffset = 0;
    } else if (currentExpression == EyesLookRight) {
      pupilOffset = 2;
    } else {
      // The pupils wander gently while the expression is neutral.
      float lookWave = sinf((float)currentMillis * TWO_PI / 7000.0f);
      pupilOffset = lookWave < -0.45f ? 0 : (lookWave > 0.45f ? 2 : 1);
    }

    drawMatrixEye(0, 2, pupilOffset, eyeColor, pupilColor);
    drawMatrixEye(5, 2, pupilOffset, eyeColor, pupilColor);
  }

  screen.show();
}

void drawMatrixEye(uint8_t x, uint8_t y, uint8_t pupilOffset, uint16_t eyeColor, uint16_t pupilColor) {
  screen.fillRect(x, y, 3, 4, eyeColor);
  screen.drawPixel(x + pupilOffset, y + 2, pupilColor);
}
