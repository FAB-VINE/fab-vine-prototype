...
void drawAscendingColorBreath(const unsigned long& currentMillis) {
  const unsigned long periodMs = 8200;
  const uint8_t ledsPerColumn = 10;
  const uint8_t columnCount = 4;

  float phase = (float)(currentMillis % periodMs) / (float)periodMs;
  float breath = (1.0f - cos(phase * TWO_PI)) * 0.5f;
  float breathHeight = breath * breath * (3.0f - 2.0f * breath);
  ...
  for (uint8_t column = 0; column < columnCount; column++) {
    for (uint8_t row = 0; row < ledsPerColumn; row++) {
      float rowHeight = (float)row / (float)(ledsPerColumn - 1);

      // Una masa de luz sube y baja con borde amplio, seguida por una estela suave.
      float fill = constrain((breathHeight - rowHeight + 0.20f) / 0.38f, 0.0f, 1.0f);
      fill = fill * fill * (3.0f - 2.0f * fill);
      float distanceToFront = (rowHeight - breathHeight) / 0.25f;
      float frontGlow = exp(-(distanceToFront * distanceToFront));
      float amount = 0.025f + (0.68f * fill) + (0.32f * frontGlow);
      amount *= 0.72f + (0.28f * breath);
      amount = constrain(amount, 0.0f, 1.0f);

      // El color también forma un gradiente vertical; no cambia todo de golpe.
      float colorPosition = fmod((phase * 3.0f) + (rowHeight * 0.85f), 3.0f);
      ...
      uint8_t level = 4 + (uint8_t)(156.0f * amount);
