...
void updateLeds(const unsigned long& currentMillis) {
  drawNeighborDirectionGuide(currentMillis);
}

void drawNeighborDirectionGuide(const unsigned long& currentMillis) {
  const uint8_t ledsPerGroup = 10;
  const uint8_t groupCount = 4;
  const unsigned long directionDurationMs = 2800;
  const Face directionOrder[FACE_COUNT] = {
    FaceFront, FaceRight, FacePosterior, FaceLeft, FaceTop, FaceBottom
  };

  uint8_t availableCount = 0;
  for (uint8_t i = 0; i < FACE_COUNT; i++) {
    if (!hasNeighbor[directionOrder[i]]) availableCount++;
  }

  // Si todas las caras están conectadas, vuelve a la respiración orgánica.
  if (availableCount == 0) {
    drawAscendingColorBreath(currentMillis);
    return;
  }

  uint8_t wantedAvailable = (currentMillis / directionDurationMs) % availableCount;
  Face targetFace = FaceFront;
  uint8_t foundAvailable = 0;
  for (uint8_t i = 0; i < FACE_COUNT; i++) {
    Face candidate = directionOrder[i];
    if (!hasNeighbor[candidate]) {
      if (foundAvailable == wantedAvailable) {
  ...
  float phase = (float)(currentMillis % directionDurationMs) / (float)directionDurationMs;
  float travel = phase * 1.35f - 0.17f;
  bool verticalDirection = targetFace == FaceTop || targetFace == FaceBottom;
  int8_t targetGroup = -1;
  uint8_t red = 255;
  uint8_t green = 255;
  uint8_t blue = 255;

  if (targetFace == FaceFront) {
    targetGroup = 0; red = 210; green = 18; blue = 8;
  } else if (targetFace == FaceRight) {
    targetGroup = 1; red = 8; green = 190; blue = 55;
  } else if (targetFace == FacePosterior) {
    targetGroup = 2; red = 5; green = 80; blue = 230;
  } else if (targetFace == FaceLeft) {
    targetGroup = 3; red = 145; green = 15; blue = 220;
  }
  ...
  for (uint8_t group = 0; group < groupCount; group++) {
    for (uint8_t row = 0; row < ledsPerGroup; row++) {
      ...
      if (verticalDirection) {
        float directedPosition = targetFace == FaceTop ? position : 1.0f - position;
        float distance = (directedPosition - travel) / 0.18f;
        glow = exp(-(distance * distance));
      } else if (group == targetGroup) {
        // Las cuatro caras laterales conservan su color y apuntan hacia arriba.
