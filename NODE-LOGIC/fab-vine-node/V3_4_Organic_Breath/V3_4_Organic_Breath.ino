...
void drawAscendingColorBreath(const unsigned long& currentMillis) {
  ...
  for (uint8_t column = 0; column < columnCount; column++) {
    for (uint8_t row = 0; row < ledsPerColumn; row++) {
      ...
      float fill = constrain((breathHeight - rowHeight + 0.22f) / 0.42f, 0.0f, 1.0f);
      fill = fill * fill * (3.0f - 2.0f * fill);
      float distanceToFront = (rowHeight - breathHeight) / 0.23f;
      float frontGlow = exp(-(distanceToFront * distanceToFront));
      float distanceToTrail = (rowHeight - breathHeight + 0.24f) / 0.34f;
      ...
      float colorPosition = fmod((phase * 3.0f) + (rowHeight * 1.05f) + (amount * 0.16f), 3.0f);
