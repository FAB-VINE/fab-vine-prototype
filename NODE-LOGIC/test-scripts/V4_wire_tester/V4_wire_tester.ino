...
void setup() {
  ...
  Serial1.begin(BAUD_RATE, SERIAL_8N1, -1, PIN_BROADCAST_TX);
...
void loop() {
  ...
  for (uint8_t i = 0; i < FACE_COUNT; ++i) {
    ...
    int maxCharacterReadperLoop = 100;
    while (rxPorts[face]->available() > 0 || maxCharacterReadperLoop <= 0) {
